...
void build(int& preIndex, int idx_root_inOrder, Node* &root, int left, int right, string sign) {
	...
	if (idx_root_child_inOrder >= left && idx_root_child_inOrder <= right) {
		...
		if (sign == "left") {
			...
			build(preIndex, idx_root_child_inOrder, root->left, left, idx_root_child_inOrder - 1, "left");
			build(preIndex, idx_root_child_inOrder, root->left, idx_root_child_inOrder + 1, right, "right");
		}
		else if (sign == "right") {
			root->right = new Node(inOrder[idx_root_child_inOrder]);
			build(preIndex, idx_root_child_inOrder, root->right, left, idx_root_child_inOrder - 1, "left");
			build(preIndex, idx_root_child_inOrder, root->right, idx_root_child_inOrder + 1, right, "right");
		}
	}
}
